...
@section('content')
    <div class="bg-white rounded-xl shadow-sm p-6 max-w-2xl">
        <h3 class="text-xl font-semibold mb-1">Tambah Kelas</h3>
        <p class="text-sm text-gray-500 mb-6">Isi data kelas, kamar, atau kelompok.</p>
        ...
        <form action="{{ route('kelas.store') }}" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <textarea name="keterangan" id="keterangan" rows="4"
                          class="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:outline-none focus:ring focus:ring-green-200">{{ old('keterangan') }}</textarea>
                @error('keterangan')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
