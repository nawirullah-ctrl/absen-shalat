...
@section('content')
    <div class="mb-6">
        <h3 class="text-xl font-semibold">Absensi Kegiatan Tambahan</h3>
        <p class="text-sm text-gray-500">Input absensi kegiatan tambahan berdasarkan tanggal dan jenis kegiatan.</p>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm p-6 mb-6">
        <form action="{{ route('absensi-kegiatan-tambahan.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-5 gap-4">
            <div>
    ...
    <form action="{{ route('absensi-kegiatan-tambahan.store') }}" method="POST">
        @csrf
        <input type="hidden" name="tanggal" value="{{ $tanggal }}">
        <input type="hidden" name="kegiatan_tambahan_id" value="{{ $kegiatanTambahanId }}">
        ...
        @if($santris->count() > 0 && $kegiatanTambahanId)
            <div class="mt-6">
