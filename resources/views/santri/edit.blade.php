...
@section('content')
    <div class="bg-white rounded-xl shadow-sm p-6 max-w-3xl">
        <h3 class="text-xl font-semibold mb-1">Edit Santri</h3>
        <p class="text-sm text-gray-500 mb-6">Perbarui data santri.</p>

        <form action="{{ route('santri.update', $santri->id) }}" method="POST" class="space-y-5">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <select name="kelas_id" id="kelas_id"
                        ...
                        @foreach($kelas as $item)
                            <option value="{{ $item->id }}" {{ old('kelas_id', $santri->kelas_id) == $item->id ? 'selected' : '' }}>
                                {{ $item->nama }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="text" name="nama_wali" id="nama_wali" value="{{ old('nama_wali', $santri->nama_wali) }}"
                           class="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:outline-none focus:ring focus:ring-green-200">
                    @error('nama_wali')
                        <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="alamat" id="alamat" rows="4"
                              class="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:outline-none focus:ring focus:ring-green-200">{{ old('alamat', $santri->alamat) }}</textarea>
                    @error('alamat')
                        <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
