...
@section('content')
    <div class="bg-white rounded-xl shadow-sm p-6 max-w-3xl">
        <h3 class="text-xl font-semibold mb-1">Tambah Kegiatan Tambahan</h3>
        <p class="text-sm text-gray-500 mb-6">Isi data kegiatan tambahan pondok.</p>

        <form action="{{ route('kegiatan-tambahan.store') }}" method="POST" class="space-y-5">
            @csrf
            ...
            <div>
                ...
                <textarea name="deskripsi" id="deskripsi" rows="4"
                          class="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:outline-none focus:ring focus:ring-green-200">{{ old('deskripsi') }}</textarea>
                @error('deskripsi')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
